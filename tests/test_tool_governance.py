...
async def test_transfer_requires_approval_bound_to_arguments_and_masks_accounts() -> None:
    ...
    assert [record.safe_target for record in audit.records] == [
        f"ACC-A-****3456->ACC-A-****4321:{amount}"
        for amount in (1200.0, 9000.0, 1200.0, 1200.0, 1200.0)
    ]
    assert FROM_ACCOUNT not in audit_json and TO_ACCOUNT not in audit_json
